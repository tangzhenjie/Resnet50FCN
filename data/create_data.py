...
def transform(image, mask, img_size, is_crop=True):
...
class GenerateData(data.Dataset):
    def __init__(self, image_dir, img_size, augment=None):
    ...
    def __getitem__(self, index):
        ...
        if self.augment:
            img, label = transform(img, label, self.img_size)
        else:
            img, label = transform(img, label, self.img_size, is_crop=False)
